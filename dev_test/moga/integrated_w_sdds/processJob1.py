...
def main(rootname, oldDir, xchrom_str, ychrom_str, tagList, valueList):
    """"""
    ...
    if False: # I don't think this check is needed and can make it more tedious
              # during debugging because I have to clean up the leftover files manually
    ...
    if Path(f'{rootname}.w1').exists() and Path(f'{rootname}.fin').exists():
        ...
        cmd_list = [
            (f'sddsprocess {rootname}.finc -pipe=out -nowarning '
             '"-define=col,deltaAbs,MAL.DP abs" '
        ...
        cmd_list = [
            ...
            f'sddsxref -pipe {rootname}.finc -take=MAL.DP',
            ('sddsprocess -pipe "-define=column,deltaAbs,MAL.DP abs" '
    ...
    # Compute the Touschek lifetime for 500 mA in 1200 bunches.
    # The local momentum aperture is capped at the value we just computed usig the -deltaLimit option
    use_py_computeLifetime = True
    current_mA = 5e2
    bunches = 1200 # ~90% fill (out of 1320 buckets)
    coupling_str = '{:.16g}'.format(0.50)
    deltaLimit_str = f'{1e2 * deltaLimit:.16g}'
    if not use_py_computeLifetime:
        cmd = (
            f'bash computeLifetime -rootname {rootname} -current {current_mA:.0f} -bunches {bunches:d} '
            f'-coupling {coupling_str} -deltaLimit {deltaLimit_str}')
    else:
        if False: # This will fail w/ Bad file descriptor error due to MPI
            # re-initialization attempt error. Must use the import method below.
            cmd = (
                f'python computeLifetime.py -rootname {rootname} -current {current_mA:.0f} -bunches {bunches:d} '
                f'-coupling {coupling_str} -deltaLimit {deltaLimit_str}')
            #print(cmd)
            p = Popen(shlex.split(cmd), stdout=PIPE, stderr=PIPE, encoding='utf-8')
            ...
            import computeLifetime
            try:
                computeLifetime.main(rootname, current_mA, bunches,
                                     coupling_str, deltaLimit_str)
    ...
    # that.
    #
    # => For NSLS-III, I will remove this clipping.
    ...
    cmd_list = [
        (f'sddsprocess {rootname}.aper -pipe=out '
         ...
         f'"-define=column,xClipped1,xClipped 0.007 > ? xClipped : xClipped $ ,units=m" '
...
if __name__ == '__main__':
    ...
    main(rootname, oldDir, xchrom_str, ychrom_str, tagList, valueList)
