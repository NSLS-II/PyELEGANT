...
def calcAnalyticalORM(
    LTE_filepath,
    E_MeV,
    bpm_elem_inds_or_name_occur_tups,
    hcor_elem_inds_or_name_occur_tups,
    vcor_elem_inds_or_name_occur_tups,
    use_beamline=None,
):
    """
    ...
    if isinstance(bpm_elem_inds_or_name_occur_tups[0], (int, np.integer)):
        ...
        bpm_inds = np.array(
            [
                np.where(
                    (twi_arrays["ElementName"] == name)
                    & (twi_arrays["ElementOccurence"] == occur)
                )[0][0]
    ...
    for i in range(nBPM):
        ...
        M[i, :nHCOR] = (
            np.sqrt(cor_betax_avg * betax[bpm_inds[i]])
            * np.cos(np.pi * nux - np.abs(cor_phix_avg - phix[bpm_inds[i]]))
            / (2 * np.sin(np.pi * nux))
        ...
        M[i + nBPM, nHCOR:] = (
            np.sqrt(cor_betay_avg * betay[bpm_inds[i]])
            * np.cos(np.pi * nuy - np.abs(cor_phiy_avg - phiy[bpm_inds[i]]))
            / (2 * np.sin(np.pi * nuy))
