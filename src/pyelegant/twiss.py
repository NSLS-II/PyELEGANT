...
from . import __version__, elebuilder, sdds, std_print_enabled, util
...
def _calc_twiss(
    ...
    if higher_order_chromaticity:
        if concat_order != 3:
            sys.stderr(
                (
                    "WARNING: When computing higher-order chromaticity, "
    ...
    disable_watch_elem_d = dict(
        name="*", type="WATCH", item="DISABLE", value=True, allow_missing_elements=True
    )
    ...
    if run_local:
        ...
        if ("pelegant" in remote_opts) and (remote_opts["pelegant"] is not False):
            print(
                '"pelegant" option in `remote_opts` must be False for Twiss calculation'
            )
...
def _calc_matrix_elem_linear_natural_chrom(twi_arrays, parameters_arrays):
    ...
    quads_bends = np.logical_or(
        twi["ElementType"] == "QUAD", twi["ElementType"] == "SBEN"
    )
    quads_bends = np.logical_or(quads_bends, twi["ElementType"] == "KQUAD")
    quads_bends = np.logical_or(quads_bends, twi["ElementType"] == "CSBEND")
    ...
    quads_bends = np.logical_or(
        para["ElementType"] == "QUAD", para["ElementType"] == "SBEN"
    )
    quads_bends = np.logical_or(quads_bends, para["ElementType"] == "KQUAD")
    quads_bends = np.logical_or(quads_bends, para["ElementType"] == "CSBEND")
    param_inds = {}
    for k in ["K1", "L", "ANGLE"]:
    ...
    output = dict(
        elem_name=elem_names,
        sb=sbs,
        se=ses,
        K1=param_vals["K1"],
        L=param_vals["L"],
        rhoinv_sqr=param_vals["rhoinv_sqr"],
        nat_ksi_x=[],
        nat_ksi_y=[],
    )
...
def plot_twiss(
    ...
    s0_m=0.0,
    etax_unit="mm",
    right_margin_adj=0.88,
    twiss_plot_area_height=5,
    print_scalars=None,
    ...
    [
        hbx,
    ] = ax1.plot(twi_ar["s"][_vis] - s0_m, twi_ar["betax"][_vis], "b.-", ms=ms_main)
    [
        hby,
    ] = ax1.plot(twi_ar["s"][_vis] - s0_m, twi_ar["betay"][_vis], "r.-", ms=ms_main)
    ax1.set_xlim(shifted_slim)
    if show_mag_prof:
        util.add_magnet_profiles(
            ax2, twi_ar, param_ar, slim, s_margin_m=s_margin_m, s0_m=s0_m
        )
        plt.setp(ax1.get_xticklabels(), visible=False)
        if s0_m == 0.0:
            ax2.set_xlabel(r"$s\, [\mathrm{m}]$", **_kw_label)
        else:
            ax2.set_xlabel(r"$s\, - {:.6g}\, [\mathrm{{m}}]$".format(s0_m), **_kw_label)
    ...
    plt.legend(
        [hbx, hby, hex],
        [r"$\beta_x$", r"$\beta_y$", r"$\eta_x$"],
        bbox_to_anchor=(0, 1.04, 1.0, 0.102),
        loc="upper center",
        mode="expand",
        ncol=3,
        borderaxespad=0.0,
    ...
    if False:
        ...
        plt.plot(
            (twi_ar["s"] - s0_m)[_vis],
            twi_ar["psix"][_vis] / (2 * np.pi),
            "b-",
            (twi_ar["s"] - s0_m)[_vis],
            twi_ar["psiy"][_vis] / (2 * np.pi),
            "r-",
        )
        plt.legend([r"$\nu_x$", r"$\nu_y$"], prop=leg_prop)
        # plt.grid(True)
