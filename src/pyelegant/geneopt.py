...
def run(
    root_name, parameterName, initialValue, lowerLimit, upperLimit, errorLevel,
    ...
    ** Columns **
    ...
    #geneticOptimizer -input optim1.sdds -reduce 1 -contin 1 -updateOnly 1
    ...
    assert len(parameterName) == len(initialValue) == len(errorLevel) == \
           len(lowerLimit) == len(upperLimit)

    assert np.all(np.array(lowerLimit) <= np.array(initialValue))
    assert np.all(np.array(initialValue) <= np.array(upperLimit))
    ...
    util.run_cmd_w_realtime_print(cmd_list, return_stdout=False, return_stderr=False)