...
def create_oct_elements_subsection(doc, elem_defs):
    """"""
...
def add_worksheet_lattice_props(workbook, wb_txt_fmts, wb_num_fmts, ws, lin_data):
    ...
    for row_spec, symbol_def, num_fmt in table_order:
        ...
        if row_spec not in ('arcs_straights_frac',):
            ...
            raise NotImplementedError(row_spec)
...
def get_lattice_prop_row(lin_data, row_spec, excel_output=False):
    ...
    if isinstance(k, list):
        ...
        if k == 'ksi_nat':
            ...
            label = 'Horizontal Natural Chromaticity'
            unit = '()'
            ...
            label = 'Horizontal Corrected Chromaticity'
            unit = '()'
...
def gen_report_obsolete(args):
    """"""

    config_filepath = args.config_filepath

    if args.full_example_config:
        gen_example_config_file(config_filepath, 'full', args.example_config_version)
    elif args.min_example_config:
        gen_example_config_file(config_filepath, 'min', args.example_config_version)
    else:
        if args.type == 0:
...
def main():
    ...
    #args = get_parsed_args_obsolete()
    #gen_report_obsolete(args)
